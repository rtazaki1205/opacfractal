...
integer                                 :: iqsca,iqcor,iquiet,i,nang
complex(kind=dp)                        :: ref
...
iquiet = 0                               ! stdout
R0     = 1e-1_dp                         ! Monomer radius (micron)
PN     = 2.0_dp ** 10.0_dp               ! Number of monomer in an aggregate
nang   = 91                              ! Angle mesh
lmd    = 1.e-1_dp                        ! wavelength (micron)
k0     = 1.04_dp                         ! fractal prefactor
df     = 1.9_dp                          ! fractal dimension
ref    = cmplx(1.4_dp,0.01_dp,kind=dp)   ! complex refractive index
!--------------------------------------------------------------------------------
! RUN OPACFRACTAL
!--------------------------------------------------------------------------------
allocate(Smat(1:4,1:2*nang-1))
call meanscatt(lmd,R0,PN,Df,k0,ref,iqsca,iqcor,iquiet,nang,&
